"""
Figure 6E: per-cancer-type hazard ratio (HR) analysis of ICB (immune checkpoint
inhibitor) outcomes.

For each cancer type with at least MIN_CANCER_TYPE_N ICB-treated patients,
tests association between TTNT (time to next treatment) and TMB, MSI, MATH,
pathway-level mutation status, gene-level mutation status, mutational
signature presence, and treatment line, using univariate Cox models fit
separately within each cancer type (BH-FDR corrected within each cancer type).
...
Input (public data release, identified by KM_ID / KM_SAMPLE_ID):
    ...
    - maf (KM00_AllSamples_Combined_Mutations.maf): mutation calls, used to derive pathway- and
      gene-level mutation status (tissue panels only)

Output:
    - icb_percancer_hr.csv: one row per (cancer_type, feature) with HR, CI,
      p-value, FDR (FDR computed within each cancer type)
"""
import numpy as np
import pandas as pd
...
DATA_DIR = "../Data"
OUTPUT_PATH = "output/icb_percancer_hr.csv"
...
MIN_CANCER_TYPE_N = 20  # cancer types with fewer ICB patients than this are skipped
...
def run_univariate_cox(df, feature, feature_type, min_n=10, min_event=5, min_positive=3):
...
def run_categorical_cox(df, cat_col, min_n=10, min_event=5, min_positive=3):
    """Dummy-encode a categorical feature and fit a univariate Cox model per level."""
    tmp = df[[TIME_COL, EVENT_COL, cat_col]].dropna()
    if len(tmp) < min_n or tmp[EVENT_COL].sum() < min_event:
        return []
...
def main():
    ...
    categorical_vars = ["Line_of_Treatment"]

    cancer_counts = icb["Abbreviation"].value_counts()
    valid_cancers = cancer_counts[cancer_counts >= MIN_CANCER_TYPE_N].index.tolist()

    all_results = []
    for cancer in valid_cancers:
        sub = icb[icb["Abbreviation"] == cancer]

        cancer_results = [run_univariate_cox(sub, f, "continuous") for f in continuous_vars]
        cancer_results += [run_univariate_cox(sub, f, "binary") for f in binary_vars]
        for f in categorical_vars:
            cancer_results.extend(run_categorical_cox(sub, f))

        for res in cancer_results:
    ...
    fdr_parts = []
    for cancer, grp in result_df.groupby("cancer_type"):
        grp = grp.copy()
        ok = grp["status"] == "ok"
        if ok.sum() > 1:
            grp.loc[ok, "FDR"] = multipletests(grp.loc[ok, "p_value"], method="fdr_bh")[1]
        else:
            grp["FDR"] = np.nan
        fdr_parts.append(grp)

    result_df = pd.concat(fdr_parts).reset_index(drop=True)
    result_df = result_df.sort_values(["cancer_type", "p_value"], na_position="last")

...
genie_meta <- c(
  ...
  "cancer_type", "female", "post_profiling", "platinum_regimen", "OS_mapping"
)
...
g_results <- map_dfr(g_cancers, function(ct) {
  dat <- filter(km, cancer_type == ct)
  candidate <- c("female", "post_profiling", feature_columns)
...
model_features <- c("female", "post_profiling", feature_columns)
...
genie$risk_bad <- score_data(genie)
...
k <- make_km(genie, "TTNT_month", "event_comp", "GENIE-BPC NSCLC",
             "Treatment durability", TRUE)
l_internal <- make_km(test, "OS_month", "OS_event", "K-MASTER test cohort",
                      "Overall survival", FALSE)
l_external <- make_km(genie, "OS_month", "OS_event", "GENIE-BPC NSCLC",
                      "Overall survival", FALSE)

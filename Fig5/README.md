...
As in the original model, patients with non-positive or non-finite TTNT are excluded
before the 70/30 modeling split.
...
The simple matrices contain the original clinical predictors (`female` and `late`, where
`late` denotes age at least 50 years), TMB, hotspot and gene-level features.

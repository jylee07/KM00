...
analyse_feature <- function(dat, feature) {
  ...
  n_mut <- sum(x == 1L)
  n_wt <- sum(x == 0L)
  if (n_mut < 2L || n_wt < 2L || length(unique(x)) < 2L || sum(event) < 2L) return(NULL)
...
# -----------------------------------------------------------------------------
set.seed(args$seed)
score_results <- map_dfr(eligible_cancers, function(ct) {
  ...
  map_dfr(seq_len(args$iterations), function(iteration_number) {
    ...
    one_iteration <- map_dfr(feature_columns, ~ analyse_feature(tmp, .x))
    if (!nrow(one_iteration)) return(tibble())
    one_iteration %>%
      mutate(
        cancer_type = ct,
        cancer_abbr = abbr(ct),
        iteration = iteration_number,
        score_z = zscore_safe(score_raw),
        .before = 1
      )
  })
})

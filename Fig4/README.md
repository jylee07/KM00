...
- The Molecular Prognostic Index follows the legacy calculation and is
  ...
  `-log2(Cox HR) * -log10(Cox Wald P) * prevalence_percent`.

  The original workflow used two different P values: Cox Wald P for the MPI magnitude
  and log-rank P for counting significant resampling iterations. The legacy table named
  the latter `cox_pval`; the public output uses the clearer name `logrank_p`.

- For each cancer type and feature, 100 independent random subsamples are drawn without
  replacement, matching the feature-wise legacy loops.
